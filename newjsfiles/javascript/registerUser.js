...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
    ...
    if (user_from_store && user_from_store.isEnrolled()) {
        console.log('Successfully loaded admin from persistence<br>');
        admin_user = user_from_store;
    } else {
        throw new Error('Failed to get admin.... run enrollAdmin.js<br>');
    }

    // at this point we should have the admin user
    // first need to register the user with the CA server

    //af = fabric_ca_client.newAffiliationService();

    return fabric_ca_client.register({enrollmentID: user_name, affiliation: '',role: 'client'}, admin_user);
}).then((secret) => {
    // next we need to enroll the user with CA server
    console.log('Successfully registered ' + user_name + ' - secret:'+ secret + '<br>');

    return fabric_ca_client.enroll({enrollmentID: user_name, enrollmentSecret: secret});
}).then((enrollment) => {
  console.log('Successfully enrolled member user ' + user_name+ '<br>');
  return fabric_client.createUser(
    ...
    //response.write("Error occured see console for the problem...");
