...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	if (user_from_store && user_from_store.isEnrolled()) {
		...
		throw new Error('Failed to get getblock'+user_name+'.... run registerUser.js');
	}
		...
		if (block[0] instanceof Error) {
			...
			return str;
  			/*block.data.data.forEach(transaction => {
				...
				console.log('Data: ');
				console.log(JSON.stringify(transaction.payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset[1].rwset.writes));
			});*/
		}
	} else {
